...
double SDALTQuery(int s, int t, string regL, bool fixLflag){
    ...
    df[s][DFAq0] = 0;
    Qf.push(pqn(prio(s,t), s, DFAq0, 0));
    ...
    while (!Qf.empty()){
        ...
        for (int i = 0; i < adjL[v].size();i++){
            ...
            if (df[e.to][toq] > df[v][q] + e.weight){
                ...
                Qf.push(pqn(df[e.to][toq] + prio(e.to, t), e.to, toq, df[e.to][toq]));
            }
        }
    }
    return -1;
...
void rep(string qi, string sfile, string &setres, string reg, bool fixflag){
    ...
    for (int i = 0; i < qn; i++){
        ...
        if(fixflag){
            ...
            tmp = SDALTQuery(queryset[i].first, queryset[i].second, reg, 1);
            t2=std::chrono::high_resolution_clock::now();
        }
        else{
            t1=std::chrono::high_resolution_clock::now();
            tmp = SDALTQuery(queryset[i].first, queryset[i].second, rega[i], 0);
            t2=std::chrono::high_resolution_clock::now();
        }
    ...
    FILE *fp_out = fopen((s3+string("SDALTResults")).c_str(), "w");
...
int main(int argc , char * argv[]){
	...
	double runT;
    t1 = std::chrono::high_resolution_clock::now();
    FILE *fp_landmarks = fopen((prefix + string("landmarks.txt")).c_str(), "r");
    FILE *fp_landmarks_dis = fopen((prefix + string("landmarksDis.txt")).c_str(), "r");
    double tval;
    fscanf(fp_landmarks, "%d", &nlm);
    for (int i = 0; i < nlm;i++){
        int tlm;
        fscanf(fp_landmarks, "%d", &tlm);
        tlm--;
        lminds.push_back(tlm);
    }
    long long indexsize = 0;
    for (int j = 1; j <= N;j++){
        vector<double> tmp;
        for (int i = 0; i < nlm;i++){
            fscanf(fp_landmarks_dis, "%lf", &tval);
            indexsize += 2;
            tmp.push_back(tval);
        }
        lm.push_back(tmp);
    ...
    FILE *fp_record = fopen("SDALTRecord.txt", "a");
    fprintf(fp_record, "%s\n", setres.c_str());
    fclose(fp_record);
    return 0;
}
